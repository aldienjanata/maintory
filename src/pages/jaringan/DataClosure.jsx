    ...
    <div className="page-container">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px', flexWrap: 'wrap', marginBottom: '20px' }}>
        <div>
          <h2 style={{ margin: 0, display: 'flex', alignItems: 'center', gap: '10px', fontSize: '20px', fontWeight: 700 }}>
            <img src="/icon_closure.png" alt="closure" style={{ width: '24px', height: '24px', objectFit: 'contain' }} onError={(e) => e.target.style.display='none'} /> Data Closure
          </h2>
          <p style={{ margin: '4px 0 0', fontSize: '13px', color: 'var(--text-secondary)' }}>Jaringan Fiber — Pencatatan & Manajemen Closure</p>
        </div>
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          {['admin', 'superadmin', 'teknisi'].includes(role) && (
            <button className="btn btn-primary btn-sm" onClick={openAdd} title="Tambah Data Closure Baru">
              <Plus size={14} /> Tambah
            </button>
          )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '10px', marginBottom: '16px' }}>
        <div className="card" style={{ padding: '12px 14px', borderTop: '3px solid var(--accent)' }}>
          ...
          <div style={{ fontSize: '11px', color: 'var(--text-secondary)', marginTop: '2px' }}>Total Closure</div>
        </div>
          ...
          {bulkMenuOpen && (
            <>
              ...
              <div style={{ position: 'absolute', top: 'calc(100% + 6px)', left: 0, zIndex: 100, background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', boxShadow: '0 8px 24px rgba(0,0,0,0.3)', minWidth: '220px', overflow: 'hidden' }}>
                ...
                <button className="dropdown-item" style={{ width: '100%', padding: '10px 14px', textAlign: 'left', background: 'rgba(239,68,68,0.08)', border: 'none', cursor: 'pointer', fontSize: '13px', color: 'var(--danger)', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '8px' }} onClick={() => { setBulkMenuOpen(false); openBulkDeleteModal('all') }}><Trash2 size={14} /> Hapus SEMUA Data Closure</button>
              </div>
            </>
          )}
        </div>
          ...
          <table className="table" style={{ minWidth: '960px', fontSize: '13px' }}>
            <thead>
              <tr>
                ...
                <th style={{ cursor: 'pointer', minWidth: '160px' }} onClick={() => handleSort('closure_id')}><div style={{ display: 'flex', alignItems: 'center', gap: '3px' }}>Closure ID <SortIcon col="closure_id" /></div></th>
                <th style={{ cursor: 'pointer' }} onClick={() => handleSort('tipe_closure')}><div style={{ display: 'flex', alignItems: 'center', gap: '3px' }}>Tipe <SortIcon col="tipe_closure" /></div></th>
                <th style={{ cursor: 'pointer' }} onClick={() => handleSort('jumlah_core')}><div style={{ display: 'flex', alignItems: 'center', gap: '3px' }}>Jumlah Core <SortIcon col="jumlah_core" /></div></th>
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={13} style={{ textAlign: 'center', padding: '30px' }}>Memuat data...</td></tr>
              ) : paginated.length === 0 ? (
                <tr><td colSpan={13} style={{ textAlign: 'center', padding: '30px', color: 'var(--text-secondary)' }}>Tidak ada data closure ditemukan</td></tr>
              ) : (
                paginated.map((p, i) => (
                  <tr key={p.id}>
                    {role === 'superadmin' && (
                    ...
                    <td style={{ fontWeight: 600, color: 'var(--text-primary)' }}>{p.closure_id}</td>
                    <td>{p.tipe_closure}</td>
                    <td>{p.jumlah_core}</td>
                    <td>{p.kecamatan}</td>
                    <td>{p.desa}</td>
        ...
        <div className="modal-overlay">
          <div className="modal-content" style={{ maxWidth: '700px' }}>
            <div className="modal-header">
              <h3>{editingId ? 'Edit Data Closure' : 'Tambah Data Closure'}</h3>
            ...
            <div className="modal-body" style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '14px' }}>
                ...
                {!editingId && (
                  <div className="form-group">
                    <label>Closure ID <span style={{ fontWeight: 'normal', color: 'var(--text-secondary)', fontSize: '11px' }}>(Opsional)</span></label>
                    <input className="form-input" value={form.closure_id_manual} onChange={e => setForm({ ...form, closure_id_manual: e.target.value })} placeholder={itemIdPreview || 'Otomatis...'} />
                    {itemIdPreview && !form.closure_id_manual && <div style={{ fontSize: '11px', color: 'var(--accent)', marginTop: '4px' }}>Preview: {itemIdPreview}</div>}
                  </div>
                )}
                <div className="form-group">
                  <label>Tipe Closure</label>
                  <select className="form-input" value={form.tipe_closure} onChange={e => setForm({ ...form, tipe_closure: e.target.value })}>
                    {TIPE_CLOSURE.map(s => <option key={s.value} value={s.value}>{s.label}</option>)}
                  </select>
                </div>
                <div className="form-group">
                  <label>Jumlah Core</label>
                  <input className="form-input" type="number" value={form.jumlah_core} onChange={e => setForm({ ...form, jumlah_core: e.target.value })} placeholder="Cth: 24" />
                </div>
              </div>
        ...
        <div className="modal-overlay" style={{ zIndex: 110 }}>
          <div className="modal-content" style={{ maxWidth: '400px' }}>
            ...
            <div className="modal-body">
              <p>Yakin ingin menghapus Closure <strong>{confirmDelete.closure_id}</strong>?</p>
