...
const DEFAULT_FORMAT = 'NAT/{SITE_CODE}/KASET/{DESA}/{NO}'

function generateKasetId(site, desa, existingItems, formatTemplate = DEFAULT_FORMAT) {
  if (!desa) return ''
  ...
  const sameDesaItems = existingItems.filter(
    p => p.site === site && p.desa?.toUpperCase().trim() === desa.toUpperCase().trim() && p.kaset_id
  )
  ...
  for (const p of sameDesaItems) {
    const match = p.kaset_id.match(idRegex)
    if (match && match[1]) {
  ...
  if (maxNo === 0 && sameDesaItems.length > 0) {
    for (const p of sameDesaItems) {
      const match = p.kaset_id.match(/\d+$/)
      if (match) {
...
export default function DataKasetFo() {
  ...
  const filtered = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      data = data.filter(p => 
        p.kaset_id?.toLowerCase().includes(q) || 
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      delete payload.kaset_id_manual

      if (editingId) {
        const { error } = await supabase.from('network_kaset_fo').update(payload).eq('id', editingId)
        if (error) throw error
        toast.success('Data Kaset FO diperbarui!')
      } else {
        const newId = form.kaset_id_manual?.trim() || generateKasetId(form.site, form.desa, items)
        const { error } = await supabase.from('network_kaset_fo').insert({ ...payload, kaset_id: newId, created_by: profile.id })
        if (error) throw error
        toast.success(`Kaset FO ${newId} ditambahkan!`)
      }
  ...
  const handleDelete = async (item) => {
    try {
      const { error } = await supabase.from('network_kaset_fo').delete().eq('id', item.id)
      if (error) throw error
      toast.success('Data Kaset FO dihapus')
  ...
  const openBulkDeleteModal = (mode) => {
    if (mode === 'selected') {
      ...
      setBulkDeleteModal({ mode, label: `${selectedIds.size} kaset yang dipilih`, filter: null })
    } else if (mode === 'all') {
      setBulkDeleteModal({ mode, label: `SELURUH DATA KASET FO`, filter: null })
    }
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      for (let i = 0; i < targetIds.length; i += chunkSize) {
        ...
        const { error } = await supabase.from('network_kaset_fo').delete().in('id', chunk)
        if (error) throw error
      }

      toast.success(`${targetIds.length} Kaset FO berhasil dihapus!`)
  ...
  const handleExportExcel = () => {
    ...
    setTimeout(() => {
      const rows = filtered.map((p, i) => {
        const linkedDev = devices.find(d => d.id === p.device_ref)
        return {
          'No': i + 1, 'Site': SITES.find(s => s.value === p.site)?.label || p.site,
          'Kaset ID': p.kaset_id || '', 
          'Jumlah Core': p.jumlah_core || '',
          'ODP/ODC ID': linkedDev ? linkedDev.device_id : '',
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Data Kaset FO')
      XLSX.writeFile(wb, `Data Kaset FO ${format(new Date(), 'dd-MM-yyyy')}.xlsx`)
  ...
  const handleDownloadTemplate = () => {
    const template = [{
      ...
      'Kaset ID': '', // Opsional
      'Jumlah Core': '12',
      'ODP/ODC ID': 'NAT/BMS/ODP/MUJUR/001',
    ...
    XLSX.writeFile(wb, `Template Import Kaset FO ${format(new Date(), 'dd-MM-yyyy')}.xlsx`)
    ...
    reader.onload = (ev) => {
      try {
        ...
        const mapped = rows.map((r, i) => {
          ...
          const devRef = (r['ODP/ODC ID'] || '').trim()
          let foundDevId = null
          if (devRef) {
            const matchDev = devices.find(d => d.device_id === devRef)
            if (matchDev) foundDevId = matchDev.id
          }

          return {
            _rowNo: i + 2,
            kaset_id_manual: (r['Kaset ID'] || '').trim(),
            site: (r['Site'] || 'banyumas').toLowerCase().trim(),
            jumlah_core: r['Jumlah Core'] ? parseInt(r['Jumlah Core'], 10) : null,
            device_ref: foundDevId,
            device_ref_raw: devRef,
    ...
    try {
      ...
      for (let i = 0; i < toImport.length; i += chunkSize) {
        ...
        const payload = chunk.map(r => {
          const kId = r.kaset_id_manual || generateKasetId(r.site, r.desa, currentItems)
          const newItem = {
            kaset_id: kId, site: r.site, jumlah_core: r.jumlah_core, device_ref: r.device_ref,
      ...
      toast.success(`${toImport.length} data Kaset FO berhasil diimport!`)
