...
export default function Adss() {
  ...
  const [form, setForm] = useState({ haspel_code: '', type: '24c', tube_type: '', brand: '', initial_meters: 4000, used_meters: 0, date_in: format(new Date(), 'yyyy-MM-dd'), note: '' })
  ...
  const openAdd = () => {
    setEditItem(null)
    setForm({ haspel_code: generateNextCode('24c'), type: '24c', tube_type: '', brand: '', initial_meters: 4000, used_meters: 0, date_in: format(new Date(), 'yyyy-MM-dd'), note: '' })
    setIsModalOpen(true)
  }

  const openEdit = (h) => {
    setEditItem(h)
    setForm({ haspel_code: h.haspel_code, type: h.type, tube_type: h.tube_type || '', brand: h.brand || '', initial_meters: h.initial_meters, used_meters: h.used_meters, date_in: h.date_in, note: h.note || '' })
    setIsModalOpen(true)
  }
  ...
  const handleSave = async () => {
    ...
    try {
      if (editItem) {
        const { error } = await supabase.from('adss_haspels').update({ ...form, status, updated_at: new Date().toISOString() }).eq('id', editItem.id)
  ...
  const handleExportExcel = async () => {
    try {
      ...
      const headers1 = ['Kode Haspel', 'Tipe', 'Jenis Tube', 'Merk', 'Tanggal Masuk', 'Meter Awal', 'Meter Terpakai', 'Sisa Meter', 'Status', 'Catatan']
      setColumnWidths(ws1, [16, 14, 16, 16, 16, 14, 16, 14, 12, 28])
      applyHeaderStyle(ws1, headers1)
      for (let i = 0; i < haspels.length; i++) {
        const h = haspels[i]
        ws1.addRow([
          h.haspel_code,
          `ADSS ${h.type?.toUpperCase()}`,
          h.tube_type || '-',
          h.brand || '-',
          h.date_in,
      ...
      const headers2 = ['Kode Haspel', 'Tipe', 'Jenis Tube', 'Merk', 'Tanggal', 'Jenis', 'Pekerjaan', 'Teknisi', 'Stok Awal (m)', 'Masuk (m)', 'Keluar (m)', 'Sisa Stok (m)', 'Lokasi/Note']
      setColumnWidths(ws2, [16, 14, 16, 16, 16, 12, 18, 28, 14, 14, 14, 14, 30])
      ...
      ;(allLogs || []).forEach(l => {
        ...
        const haspelType = haspelMap[hId]?.type ? `ADSS ${haspelMap[hId].type.toUpperCase()}` : '-'
        const tubeType = haspelMap[hId]?.tube_type || '-'
        const brand = haspelMap[hId]?.brand || '-'
      ...
      ;(allExpItems || []).forEach(ei => {
        ...
        const typeRaw = ei.haspel?.type || haspelMap[hId]?.type
        const haspelType = typeRaw ? `ADSS ${typeRaw.toUpperCase()}` : '-'
        const tubeType = ei.haspel?.tube_type || haspelMap[hId]?.tube_type || '-'
        const brand = ei.haspel?.brand || haspelMap[hId]?.brand || '-'
        const techNames = (ei.expense?.technicians || []).map(tid => usersMap[tid]).filter(Boolean).join(', ')
        const wType = ei.expense?.work_type
      ...
      for (let i = 0; i < rows2.length; i++) {
        const r = rows2[i]
        ws2.addRow([r.code, r.type, r.tube_type, r.brand, r.date, r.jenis, r.work, r.tech, r.stok_awal, r.masuk, r.keluar, r.stok_akhir, r.note])
        if (i % 20 === 0) {
    ...
    <div>
      <div className="page-header">
        <div className="page-header-left">
          <h2>ADSS Kabel</h2>
          <p>Kelola inventaris kabel Adss berdasarkan haspel</p>
        </div>
        <div className="page-header-right">
          {can(role, 'inventory.Adss.export') && (
            <button className="btn btn-secondary" onClick={handleExportExcel}><Download size={16} /> Export</button>
          )}
          {can(role, 'inventory.Adss.add') && (
      ...
      <div className="stats-grid mb-4">
        {[
          { key: 'total', label: 'Total Haspel Utuh', value: haspels.filter(h => (Number(h.initial_meters || 0) - Number(h.used_meters || 0)) === Number(h.initial_meters)).length, color: 'var(--accent)' },
      ...
      <div className="card">
        ...
        <div className="table-container">
          {loading ? (
                  ...
                  {paginated.map(h => {
                    ...
                    return (
                      <tr key={h.id}>
                        ...
                        <td><span className="badge badge-purple">{h.type?.toUpperCase()}</span></td>
                        <td>{h.tube_type || '-'}</td>
                        <td>{h.brand || '-'}</td>
            ...
            <div className="empty-state"><Cable size={48} /><h3>Tidak Ada Haspel</h3><p>Belum ada data haspel Adss.</p></div>
          )}
        </div>
      </div>
        ...
        <div className="modal-overlay">
          <div className="modal">
            <div className="modal-header">
              <h3>{editItem ? 'Edit Haspel' : 'Tambah Haspel Adss'}</h3>
              ...
              <div className="grid-2">
                ...
                <div className="form-group">
                  ...
                  <select className="form-input" style={{ height: 'auto' }} value={form.type} onChange={e => {
                    ...
                    <option value="144c">ADSS 144C</option>
                  </select>
                </div>
              </div>
              <div className="grid-2">
                <div className="form-group">
                  <label className="form-label">Jenis Tube</label>
                  <input className="form-input" placeholder="Contoh: Single Tube" value={form.tube_type} onChange={e => setForm(f => ({ ...f, tube_type: e.target.value }))} />
                </div>
                <div className="form-group">
                  <label className="form-label">Merk</label>
                  <input className="form-input" placeholder="Contoh: Voksel" value={form.brand} onChange={e => setForm(f => ({ ...f, brand: e.target.value }))} />
                </div>
              </div>
              <div className="grid-2">
                <div className="form-group">
