...
export default function BarcodeScanner() {
  ...
  const hasFilter = searchTerm || categoryFilter !== 'all' || filterFirstFrom || filterFirstTo || filterLastFrom || filterLastTo || kondisiFilter !== 'all' || scannerFilter !== 'all'
  const resetFilters = () => { setSearchTerm(''); setCategoryFilter('all'); setFilterFirstFrom(''); setFilterFirstTo(''); setFilterLastFrom(''); setFilterLastTo(''); setKondisiFilter('all'); setScannerFilter('all'); setCurrentPage(1) }
  ...
  useEffect(() => { setCurrentPage(1) }, [searchTerm, categoryFilter, filterFirstFrom, filterFirstTo, filterLastFrom, filterLastTo, kondisiFilter, scannerFilter, PAGE_SIZE])
                        ...
                        <td>
                          ...
                          {s.category === 'ONT' && (s.ont_kondisi || s.ont_asal || s.ont_tujuan) && (
                            <div style={{ display: 'flex', gap: '4px', marginTop: '4px', flexWrap: 'wrap' }}>
                              {s.ont_kondisi && <span style={{ fontSize: '10px', padding: '2px 6px', background: s.ont_kondisi === 'Aman' ? 'rgba(34,197,94,0.1)' : (s.ont_kondisi === 'Dismantle' ? 'rgba(245,158,11,0.1)' : 'rgba(239,68,68,0.1)'), color: s.ont_kondisi === 'Aman' ? 'var(--success)' : (s.ont_kondisi === 'Dismantle' ? 'var(--warning)' : 'var(--danger)'), borderRadius: '4px' }}>{s.ont_kondisi}</span>}
                    ...
                    <tbody>
                      {historyData.map((h, i) => (
                        <tr key={h.id} style={{ borderBottom: '1px solid var(--border)', background: i % 2 === 0 ? 'transparent' : 'var(--bg-primary)' }}>
                          ...
                          <td style={{ padding: '8px 6px' }}>
                            {h.ont_kondisi
                              ? <span style={{ fontSize: '10px', padding: '2px 6px', background: h.ont_kondisi === 'Aman' ? 'rgba(34,197,94,0.15)' : (h.ont_kondisi === 'Dismantle' ? 'rgba(245,158,11,0.15)' : 'rgba(239,68,68,0.15)'), color: h.ont_kondisi === 'Aman' ? 'var(--success)' : (h.ont_kondisi === 'Dismantle' ? 'var(--warning)' : 'var(--danger)'), borderRadius: '4px' }}>{h.ont_kondisi}</span>
