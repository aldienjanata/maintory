...
for (const file of files) {
    ...
    content = content.replace(/<div className="card" style={{ overflow: 'hidden' }}>/, 
        <div className="card" style={{ overflow: 'hidden', display: 'flex', flexDirection: 'column', minHeight: (! && .length === 0) ? 'calc(100vh - 280px)' : 'auto' }}>);
    ...
    content = content.replace(/<div style={{ padding: '40px', textAlign: 'center', color: 'var\(--text-secondary\)' }}>Tidak ada data/g, 
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px', color: 'var(--text-secondary)' }}>Tidak ada data);
    
    content = content.replace(/<div style={{ padding: '40px', textAlign: 'center', color: 'var\(--text-secondary\)' }}>Belum ada data/g, 
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px', color: 'var(--text-secondary)' }}>Belum ada data);
