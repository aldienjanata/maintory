...
content = content.replace(/filteredTickets\.map/g, 'paginated.map');
...
// Add computed if not present
if (!content.includes('const paginated = filteredTickets.slice')) {
    content = content.replace(/(return \()/, `const totalPages = Math.ceil(filteredTickets.length / ITEMS_PER_PAGE)\n  const paginated = filteredTickets.slice((currentPage - 1) * ITEMS_PER_PAGE, currentPage * ITEMS_PER_PAGE)\n\n  $1`);
}

// Check if pagination UI exists
...
`;
    // Insert after closing tags
    content = content.replace(/<\/div>\s*<\/>\s*\)\s*:\s*\(/, `${ui}\n              </div>\n            </>\n          ) : (`);
}

fs.writeFileSync(file, content);
console.log('Fixed Maintenance.jsx');
